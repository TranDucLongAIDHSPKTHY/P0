...
class LightGCN_pp(nn.Module):
    ...
    def get_sparse_graph(self):
        try:
            cache_path = adjacency_cache_file(
                self.dataset.path, "lightgcn_pp", self.alpha, self.beta
            )
            norm_adjacency = sp.load_npz(cache_path)
            logger.info("Adjacency matrix loading completed: pre_A_%s_%s.npz", self.alpha, self.beta)
        except Exception as error:
            if isinstance(error, FileNotFoundError):
                logger.info(
                    "LightGCN++ adjacency cache not found; building pre_A_%s_%s.npz",
                    self.alpha,
                    self.beta,
                )
            else:
                logger.exception(
                    "Unable to load LightGCN++ adjacency cache; rebuilding pre_A_%s_%s.npz",
                    self.alpha,
                    self.beta,
                )
            ...
            sp.save_npz(
                adjacency_cache_file(
                    self.dataset.path, "lightgcn_pp", self.alpha, self.beta, suffix=False
                ),
                norm_adjacency,
            )
            logger.info("Adjacency matrix constructed: pre_A_%s_%s.npz", self.alpha, self.beta)
